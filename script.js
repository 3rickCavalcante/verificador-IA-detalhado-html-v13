...
function generateRecommendations(humanProb) {
    let recs = [];
    if (humanProb < 40) recs = ['🔍 Realize uma entrevista oral sobre o conteúdo.','📚 Solicite uma versão anotada com comentários pessoais.','⚖️ Use esta análise como indicativo, não como prova definitiva.'];
    else if (humanProb < 70) recs = ['📝 Peça ao aluno que explique partes específicas do texto.','🔎 Compare com trabalhos anteriores do mesmo estudante.','📊 Utilize outras ferramentas complementares.'];
    else recs = ['✅ Parece ser autoria humana. Considere elogiar a qualidade.','📖 Recomende que continue desenvolvendo sua escrita acadêmica.'];
